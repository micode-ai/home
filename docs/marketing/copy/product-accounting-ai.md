...
## Расхождение «20+ / 80+ / 82» — разобрано, источник исправлен

Сайт противоречил сам себе. Что было:

| место | что было написано |
|---|---|
...
Правда — 58/15/9/82: единственная версия, которая подтверждается телом статьи,
её FAQ и диаграммой `accounting-tools-map`. «80+» в заголовке — округление 82
вниз, формально не ложь, но числа «80» в теле статьи нет вообще, поэтому в
креативах оно не используется (и не прошло бы проверку цифр).

«20+» было **устаревшим и неверным**: узлов в этом subgraph пять, а инструментов
58–82, то есть страница продукта занижала сама себя вчетверо. Исправлено на
`Up to 82 Accounting Tools` — правка в `src/data/langgraph-diagrams.ts`.

Для будущих кампаний это снимает прежний запрет: снимок с этой диаграммы больше
не ставит в кадр число, которому противоречит текст колоды. Но выбирать её
по-прежнему нужно по размеру, а не по желанию — см. правило подбора картинки в
`README.md`. Подробности выбора для этой кампании — в
