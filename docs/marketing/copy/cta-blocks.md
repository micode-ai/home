...
Раздел `## Threads` в файле текстов кампании — единственный, где пост **не
влезает целиком**: лимит 500 знаков, поэтому текст пишется цепочкой из
нескольких постов, а ссылка ставится только в последнем. Список каналов, под
которые раздел обязателен, тесты выводят из `content-plan.md`, так что раздел
может быть готов раньше слота в календаре.
