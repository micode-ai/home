...
и `tests/test_campaign_cost_of_ai_agent.py` проверяет — английские
`story-9x16-01..06.png` и `reel.mp4` тоже. Запуск с одним `pl` собрал бы
только польские и оставил английские постеры протухшими; ровно эта команда
раньше и стояла в этом файле. Без аргументов языка все три генератора теперь
одинаково берут `pl en` по умолчанию, а неизвестный язык (`ru`) отвергается
сразу, до записи первого файла, с внятным сообщением вместо `KeyError`.
...
его строкой в `test_campaign_<id>.py`, а не оставлять формат, про который
