...
export const articleTables: Record<string, Record<Lang, ArticleTable>> = {
  ...
  'jev-audit-nodes': {
    pl: {
      headers: ["Produkt","Węzeł decyzyjny","Warianty odpowiedzi","Werdykt"],
      rows: [
        ["accounting-ai", "Czy wywołać narzędzie (co krok pętli agenta)", "2 — tak / nie", "Kandydat — najwyższa częstotliwość, bo co krok, nie co wiadomość"],
        ["budget-assistant", "Typ akcji", "3 — odpowiedź / odczyt / zapis", "Kandydat"],
        ["budget-assistant", "Potwierdzenie użytkownika", "2 — tak / nie", "Nie dotyczy — to wejście od człowieka, nie decyzja modelu"],
        ["emarketing-ai", "Który z 7 specjalistów przejmuje zadanie", "8 — odpowiedź wprost + 7 agentów", "Silny kandydat — najczęstsze wywołanie na wiadomość w całym stosie"],
        ["testing-ai", "Który z 5 agentów testowych uruchomić", "5", "Niepewne — diagram nie mówi, czy to LLM, czy routing po triggerze"],
        ["testing-ai", "Czy wygenerowany test jest poprawny", "2 — dopracuj / gotowe", "Zostaje przy LLM — ocena wygenerowanego kodu"],
        ["testing-ai", "Czy wygenerowana checklista jest poprawna", "2 — dopracuj / gotowe", "Zostaje przy LLM — ocena wygenerowanej treści"],
        ["legalka-kb", "Typ wiadomości", "3 — pytanie / /suggest / recenzja KB", "Kandydat, prawdopodobnie dziś rozpoznawany bez LLM"],
        ["legalka-kb", "Czy baza wiedzy pokrywa pytanie", "2 — odpowiedz / wstrzymaj się", "Silny kandydat — dziś próg podobieństwa kosinusowego, nie model"],
        ["legalka-kb", "Intencja: pytanie czy edycja", "2", "Kandydat"],
        ["legalka-kb", "Czy poprawiona treść prawna jest poprawna", "3 — błąd / ok / przerwij", "Zostaje przy LLM — ocena merytoryczna tekstu"],
        ["legalka-kb", "Filtr danych osobowych (fail-closed)", "2", "Nie ruszamy — bramka bezpieczeństwa musi być deterministyczna"],
      ],
    },
    en: {
      headers: ["Product","Decision node","Answer options","Verdict"],
      rows: [
    ...
    ru: {
      headers: ["Продукт","Узел решения","Варианты ответа","Вердикт"],
      rows: [
        ["accounting-ai", "Вызвать инструмент (на каждом шаге цикла)", "2 — да / нет", "Кандидат — самая высокая частота: срабатывает на каждом шаге, а не на каждом сообщении"],
        ["budget-assistant", "Тип действия", "3 — ответ / чтение / запись", "Кандидат"],
        ["budget-assistant", "Подтверждение пользователя", "2 — да / нет", "Не применимо — это ввод от человека, а не решение модели"],
        ["emarketing-ai", "Какой из 7 специалистов берёт задачу", "8 — прямой ответ + 7 агентов", "Сильный кандидат — самый высокий объём вызовов на сообщение во всём стеке"],
        ["testing-ai", "Какой из 5 тестовых агентов запустить", "5", "Неясно — диаграмма не говорит, LLM это или маршрутизация по триггеру"],
        ["testing-ai", "Корректен ли сгенерированный тест", "2 — доработать / готово", "Остаётся за LLM — оценка сгенерированного кода"],
        ["testing-ai", "Корректен ли сгенерированный чек-лист", "2 — доработать / готово", "Остаётся за LLM — оценка сгенерированного контента"],
        ["legalka-kb", "Тип сообщения", "3 — вопрос / /suggest / ревью базы", "Кандидат, хотя сегодня, вероятно, определяется без LLM"],
        ["legalka-kb", "Покрывает ли база знаний вопрос", "2 — ответить / воздержаться", "Сильный кандидат — сегодня это порог косинусного сходства, не модель"],
        ["legalka-kb", "Намерение: вопрос или правка", "2", "Кандидат"],
        ["legalka-kb", "Корректен ли исправленный правовой текст", "3 — ошибка / ок / прервать", "Остаётся за LLM — содержательная оценка текста"],
        ["legalka-kb", "Фильтр персональных данных (fail-closed)", "2", "Не переносим — предохранительный барьер обязан быть детерминированным"],
      ],
    },
  },
};
