...
// Replace og:image, twitter:image and the JSON-LD "image" field with an
// article-specific image. Regex, not /g — a blog post's JSON-LD has exactly
// one "image" key (BlogPosting), so a single replace is enough and can't
// accidentally touch an unrelated "image" field elsewhere in the document.
function injectOgImage(html, absoluteUrl) {
  return html
    .replace(/(<meta property="og:image" content=")[^"]*(")/, (_, a, b) => a + absoluteUrl + b)
    .replace(/(<meta name="twitter:image" content=")[^"]*(")/, (_, a, b) => a + absoluteUrl + b)
    .replace(/("image":\s*")[^"]*(")/, (_, a, b) => a + absoluteUrl + b);
}

// Per-article, per-locale OG image generated by scripts/generate-og-images.mjs
// (written to public/og/, copied verbatim into dist/og/ by `vite build`).
// Returns null (meaning: keep whatever image the static template already has,
// i.e. the shared og-image.png) if generation failed/skipped for this slug.
function ogImageFor(slug, lang) {
  const relPath = `og/${slug}-${lang}.png`;
...
async function run() {
  ...
  const routes = [
    ...
    ...blogPosts.map((post) => ({
      ...
      // (title*) and card summary (summary*) are never touched by this. ogImage is looked up
      // independently of title/description and applies to every locale including pl.
      metaFor: (l) => {
        const ogImage = ogImageFor(post.slug, l);
        if (l === 'pl') return ogImage ? { ogImage } : null;
        return {
